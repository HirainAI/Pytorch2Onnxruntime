...
static void TestInference(Ort::Env& env, const std::basic_string<ORTCHAR_T>& model_uri,
  ...
  const int64_t input_shape[] = {1, 16384,4};
  const size_t input_shape_len = sizeof(input_shape) / sizeof(input_shape[0]);
  const size_t model_input_len = 65536 * sizeof(float);

  OrtValue* input_tensor = NULL;
  ORT_ABORT_ON_ERROR(g_ort->CreateTensorWithDataAsOrtValue(memory_info, const_cast<float*>(inputs[0].values.data()), model_input_len, input_shape,
